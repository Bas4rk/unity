...
public class sword_Attack4 : StateMachineBehaviour
{
    const float ATK_START=0.1f;
    const float ATK_END=0.4f;
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        //sword.enabled = true;
        if(ATK_START<=movingTime&&movingTime<=ATK_END){ //　ふぁせふぁせふぁせふぁｓｆmovingtime おかしい　どこで設定されてるか確認
            sword.enabled = true;
        }else{
            sword.enabled = false;		
        }
